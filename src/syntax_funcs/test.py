...
def loop(text, start, syntaxResult, symbol_table, function_table):
    ...
    for line in range(start, len(text.splitlines())):
        ...
        if in_loop:
            ...
            try:
                statement_result = statement(text, line, syntaxResult, symbol_table, function_table)
                syntaxResult = statement_result[0]
                if statement_result[1] is not None:
                    line = statement_result[1]  # Update the line index based on statement parsing
            except Exception as e:
